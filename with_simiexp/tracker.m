function [results] = tracker(p, im, bg_area, fg_area, area_resize_factor, saimese)
...
for frame = 1:num_frames
    if frame>1
        ...
        if Mode == 1
            ...
            expert = calculateRelativeSimilarity(expert, expertNum, meanFirstSim, mfr, frame, holdparam);
...
%                     IDensemble(i) = expert(i).RobScore;
            ...
            if max_sim < mfr && low_count >= lowthreshold
                % 大尺度搜索
                choosenSimExp = 1;
                low_count = 0;
                Mode = 0;
                ...
                if search_count >= searchthreshold || max_sim > mfr
                    Mode = 1;
                    search_count = 0;
                end
            else
